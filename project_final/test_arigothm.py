...
def generate_neighbors(row, col, current_board):
    ...
    return neighbors
# def solve_sudoku_hill(current_sudoku_board):
#     while True:
#         # Tìm ô trống với số lựa chọn ít nhất
#         if is_goal_state(current_sudoku_board):
#             return current_sudoku_board  # Đã giải xong Sudoku
#         if find_empty_cell(current_sudoku_board) is not None:
#             row, col = find_empty_cell(current_sudoku_board)
#         # Chọn giá trị ngẫu nhiên từ các giá trị có thể
#             choices = get_choices(row, col, current_sudoku_board)
#             if choices:
#                 current_sudoku_board[row][col] = random.choice(choices)
#                 draw_board(screen)
#                 pygame.display.flip()
#                 time.sleep(0.01) 
#             else:
...
# def get_choices(row, col, current_sudoku_board):
    x= int(math.sqrt(len(current_sudoku_board)))
    choices = set(range(1, len((current_sudoku_board))))
    choices -= set(current_sudoku_board[row])
    choices -= set(current_sudoku_board[i][col] for i in range(len(current_sudoku_board)))
    choices -= set(current_sudoku_board[i][j] for i in range((row//x)*x, (row//x)*x + x) for j in range((col//x)*x, (col//x)*x + x))
    return list(choices)
# def solve_sudoku_ucs(current_sudoku_board):
    length = len(current_sudoku_board[0])

    min_cost, min_cell = float('inf'), None
    ...
    if min_cell:
        ...
        pygame.display.flip()
        time.sleep(0.01)
    return not any(0 in row for row in current_sudoku_board)
