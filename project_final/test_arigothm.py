...
def solve_sudoku_bfs(current_sudoku_board):
    global count_step
    queue = deque([(current_sudoku_board, 0, 0)])  # (board, row, col)
    ...
    while queue and not found_solution:
        ...
        for digit in range(1,len(current_board)+1):
            is_valid_digit = is_valid(row, col, digit, current_board)
            if is_valid_digit:
                    break
        solution_board = is_goal_state(current_board)
        if solution_board and is_valid_digit:
        ...
        if find_empty_cell(current_board) is not None:
            ...
            for digit in range(1, len(current_board)+1):
                if is_valid( row, col, digit,current_board):
                    ...
                    current_sudoku_board[row][col] = digit
                    count_step += 1
                    draw_board(screen)
                    pygame.display.flip()
                    time.sleep(0.01) 
...
def solve_sudoku_ucs(current_sudoku_board):
    ...
    while queue:
        ...
        if is_goal_state(current_board):
            # The board is complete, return the solution
